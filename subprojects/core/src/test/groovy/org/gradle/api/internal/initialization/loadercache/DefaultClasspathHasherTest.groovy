...
class DefaultClasspathHasherTest extends Specification {
    ...
    def classpathHasher = new DefaultClasspathHasher(new DefaultFileHasher())

    def "classpaths are different if file contents are different"() {
        def file1 = temp.file("a.txt") << "a"
        def file2 = temp.file("b.txt") << "b"

        def a = classpathHasher.hash(new DefaultClassPath(file1))
        def b = classpathHasher.hash(new DefaultClassPath(file2))
    ...
    def "classpaths content hash are equal when file names don't match"() {
        ...
        def a = classpathHasher.hash(new DefaultClassPath(fa))
        def b = classpathHasher.hash(new DefaultClassPath(fb))
    ...
    def "classpaths are different when files have different order"() {
        ...
        def a = classpathHasher.hash(new DefaultClassPath(fa, fb))
        def b = classpathHasher.hash(new DefaultClassPath(fb, fa))
    ...
    def "classpaths are equal if all files are the same"() {
        ...
        def a = classpathHasher.hash(new DefaultClassPath(files))
        def b = classpathHasher.hash(new DefaultClassPath(files))
    ...
    def "classpaths are equal if dirs are the same"() {
        ...
        def a = classpathHasher.hash(new DefaultClassPath(temp.createDir("dir")))
        def b = classpathHasher.hash(new DefaultClassPath(temp.createDir("dir")))
    ...
    def "classpaths are the same if different if dir names are different"() {
        ...
        def a = classpathHasher.hash(new DefaultClassPath(temp.createDir("dir1")))
        def b = classpathHasher.hash(new DefaultClassPath(temp.createDir("dir2")))
    ...
    def "empty snapshots are the same"() {
        when:
        def s1 = classpathHasher.hash(new DefaultClassPath(new File(temp.createDir("dir1"), "missing")));
        def s2 = classpathHasher.hash(new DefaultClassPath(new File(temp.createDir("dir2"), "missing")));
